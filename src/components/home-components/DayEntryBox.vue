...
<template>
  <div v-if="dayEntry && (dayEntry.arrival || dayEntry.departure)" class="flex flex-col place-content-center place-items-center rounded bg-neutral-800 text-white text-xl font-bold transition-all ease-in-out p-10 px-5 select-none">
    <div class="w-full h-full">
      ...
      <div class="grid grid-rows-2 grid-cols-1 gap-10 place-items-stretch mx-auto w-full h-2/3 rounded">
        <div v-if="dayEntry.arrival" class="al-box-bubble border-pink-200">
          <div class="flex text-center place-items-center justify-center bg-pink-500">
            <i class="pi pi-stopwatch text-center" style="font-size: 1rem"></i>
          </div>
        ...
        <div v-if="dayEntry.departure" class="al-box-bubble border-indigo-200">
